    ...
    <Navbar fluid rounded>
      ...
      <>
        <div className="flex md:order-2 " >
          <Cart />
          <Link href='/profile' className='self-center mx-4 font-medium hover:underline hover:font-bold'>Hi, {session?.nickName}</Link>
          <Dropdown
            arrowIcon={false}
            inline
            label={
              <Avatar alt="User settings" img={""} rounded placeholderInitials="RR" bordered/>
            }>
            <Dropdown.Header>
              <span className="block text-sm text-center font-medium">{session?.fullName}</span>
            ...
            <Dropdown.Item as={Link} href='/profile'>Settings</Dropdown.Item>
            {session?.isSuperAdmin ? <DropdownItem as={Link} href='/admin'>Admin Controls</DropdownItem> : null}
            <Dropdown.Divider />
        ...
        <Navbar.Collapse>
          {
            pages.map((page,ind) => 
            <Navbar.Link href={page.path} active={path===page.path} key={ind}>
              <span className={`block text-sm text-center font-medium ${(path===page.path)? 'border-0 border-b-2' : null}`}>{page.title}</span></Navbar.Link> )
          }
